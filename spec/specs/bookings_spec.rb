describe "bookings", type: :request do
  ...
  describe "#update" do
    it "should update a booking and return a 204" do
      ...
      patch "/bookings/#{data["id"]}", params: {
                                         booking: { **data, dates: [*data["dates"], new_date] },
                                       },
                                       headers: { Authorization: "Bearer #{token}" }
      expect(response.status).to eq(204)
      get "/bookings/#{data["id"]}", headers: { Authorization: "Bearer #{token}" }
      data = JSON.parse response.body
  ...
  describe "#delete" do
    it "should delete a booking and return a 204" do
      create_user
      token = login_user
      create_room token
      create_booking token
      data = JSON.parse response.body
      delete "/bookings/#{data["id"]}", headers: { Authorization: "Bearer #{token}" }
      expect(response.status).to eq 204
    end
  end
end
